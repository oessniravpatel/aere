...
{ 
	...
	$statusN = 0;
	 ...
	 $query="SELECT RegisterId,IsActive FROM tblregister WHERE RegisterId='$id' AND ConfirmationCode='$code' LIMIT 1";
	$res=mysql_query($query);
	 $nubrow=mysql_num_rows($res);
	if($nubrow > 0)
	{
		...
		{
			$query="UPDATE tblregister SET IsActive='$statusY', ConfirmationCode='' WHERE RegisterId=$id";
			mysql_query($query);
			
			$msg = "
		           <div class='alert alert-success col-md-offset-4 col-md-4'>
				 			 Hey! Welcome and thank you for registering with AERE. Your account is now activated. Make the most of your registration by registering courses.
			       </div>
			       ";	
		}
		else
		{
			$msg = "
		           <div class='alert alert-error col-md-offset-4 col-md-4'>
					  <strong>Hey !</strong> Your account is already been activated <i class='fa fa-check-circle-o'></i> If you get this message in error, please contact Manny@AEREExperts.com </div>
			       ";
		}
	}
	else
...
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" />
        <title>AERE | Registration Confirmation</title>
	  ...
	  <div class="section-white section-one">
								<article class="cf wrap post-410 team type-team status-publish has-post-thumbnail hentry profile-category-manager" role="article">
									...
									<header class="article-header wow fadeInUp">
										<h1 class="page-title">Registration Confirmation</h1>
									</header>

									<section class="entry-content cf">
										<div class="m-all t-all d-all">
										
											<div class="form-bottom forgot row">
											<?php if(isset($msg)) { echo $msg; } ?>
														
											</div>
											
											<div class="center" style="text-align:center"><a href="allcourses.php" class="button btn-red btn-purple">Course Offerings</a> </div>
