    ...
    if (gblAudioFilename != '')
    {
        if (!currScreen.streamNarration)
        {
		    ...
		    else if (isIPad)
		    {
				...
				el.innerHTML = '&nbsp;<div id="jp_container_1" onclick="PlayNarration(\'' + gblAudioFilename + '\')" class="jp-audio"><div class="jp-gui jp-interface"><ul class="jp-controls"><li><a href="#play" class="jp-play" tabindex="1">play</a></li><li><a href="#play" class="jp-pause" tabindex="1">pause</a></li></ul></div></div><div id="UDUTU_Narration_div" class="jp-jplayer">'+gblAudioFilename+'</div>';
...
function jumpToIsVisited() 
{
	for (var i = 0; i < 6; i++)
	{
		var tmpDoc = document.getElementById("UDUTU_JumpTo_Option" + i +"_Content");
		if (tmpDoc == null || tmpDoc.innerHTML == ""){
			continue; 
		}
		var tmpHTML = tmpDoc.innerHTML;
		var re = /UDUTU_JumpToClicked\((\d*)\)/;
		var match = re.exec(tmpHTML); 
		var targetID = match[1];

		var targetScreen = UDUTU_API.getScreenByScreenID(targetID);
		var targetVisited = targetScreen.visited;

		if (targetVisited)
		{
			var jumpToElement = document.getElementById("UDUTU_JumpTo_Option" + i + "_Content");
			var jumpToLabel = jumpToElement.innerHTML.match(/<span>.*<\/span>/);
			jumpToElement.innerHTML = '<a class="jumpToLink" style="opacity: 0.6;filter: alpha(opacity=60);" href="javascript:UDUTU_JumpToClicked(' + targetID + ')"><span>'+ jumpToLabel[0] + '</span></a>';
		}
	}
}
