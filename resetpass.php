...
if(empty($_GET['id']) && empty($_GET['code'])){
	...
	header("refresh:10;allcourses.php");
}
...
if(isset($_GET['id']) && isset($_GET['code'])){
	...
	if(!empty($rows)){
		if(isset($_POST['btn-reset-pass']))
		{
			...
			if($cpass!==$pass)
			{
				$msg = "<div class='alert alert-block col-md-offset-4 col-md-4'>
						<button class='close' data-dismiss='alert'>&times;</button>
						<strong>Sorry!</strong>  Password and confirm password doesn't match. 
						</div>";
			}
			else
			{
				...
				$msg = "<div class='alert alert-success col-md-offset-4 col-md-4'>
						<button class='close' data-dismiss='alert'>&times;</button>
						AERE account password Changed. 
						</div>";
				header("refresh:15;allcourses.php");
			}
		}	
	}
	else
